...
contract SpicyPeno is ERC721A, ERC721ABurnable, Ownable, Pausable {
    ...
    ) ERC721A("Secret Laboratory Experiment: Spicy Peno", "PENO") {
    ...
    function mintExperiment(address testSubject, uint256 testAmount) external {
        require((totalSupply() + testAmount) <= maxSupply, "Mint is over");
        require(testAmount >= 1, "You must mint at least 1 NFT");
        require(testAmount <= 15, "You can mint up to 15 Dwards per transaction");
         _safeMint(testSubject, testAmount);
    }
